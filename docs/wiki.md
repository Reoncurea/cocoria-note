...
ここで記録した内容は、すべて報告書に出ます。

| 報告書の場所 | 内容 |
| --- | --- |
| 1ページ目「赤ちゃんの様子」 | 睡眠の時間帯と長さ（合計つき）、体温・機嫌・メモの一覧 |
| 2ページ目「呼吸チェック表」 | マスの色を3状態で再現。オレンジ（↺）＝仰向けに直した |
| 同上 | 表の下に「うつぶせから仰向けに直した時刻（2回）: 10:15、11:20」 |

報告書では機嫌を**絵文字なしの文字だけ**で出しています（印刷したときに崩れないため）。
記録が1件も無いセクションは、報告書に出ません。
